...
describe('Vault', function () {
    ...
    it('should deposit funds with correct grant information', async () => {
        const unlockDurationMins = 5; // Unlock duration in minutes

        // Calculate the unlock time (5 minutes from the current timestamp)
        const unlockTime = Math.floor(Date.now() / 1000) + unlockDurationMins * 60;
        
        const amount = ethers.parseEther("1");
        
        // Deposit 1 ether with a release time of 5 minutes
        ...
        assert.equal(grantInfo.unlockTime.toString(), unlockTime, "Unlock time should be 5 minutes from now");
    ...
    it("should allow beneficiary to claim funds after release time", async () => {
        const unlockDurationMins = 6 // Unlock duration in minutes

        // Calculate the unlock time (6 minutes from the current timestamp)
        const unlockTime = Math.floor(Date.now() / 1000) + unlockDurationMins * 60;
